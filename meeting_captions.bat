...
if not exist "scripts\meeting_captions_newest_first.py" (
  echo [ERROR] scripts\meeting_captions_newest_first.py was not found.
...
echo.
"%PY%" scripts\meeting_captions_newest_first.py
set "EXITCODE=%ERRORLEVEL%"
if not "%EXITCODE%"=="0" (
  echo.
  echo If your meeting app uses a different output device, run:
  echo   "%PY%" scripts\meeting_captions_newest_first.py --list-devices
  echo then launch with:
  echo   "%PY%" scripts\meeting_captions_newest_first.py --device DEVICE_NUMBER
  echo.
  pause
)
exit /b %EXITCODE%
